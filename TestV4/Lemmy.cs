...
namespace TestV4 {
    ...
    public class Lemmy {
        ...
        public void TestLogin() {
            ...
            Assert.That(string.IsNullOrWhiteSpace(lemmyContext.Token), Is.False, "The token has not been set for lemmy login.");
        }
        ...
        public void PostNotification_Test() {
            ...
            var taskPostNotificationInformation = PostNotification(notificationInformation, NotificationType.Information.ToString());
            ...
            var taskPostNotificationActionRequired = PostNotification(notificationActionRequired, NotificationType.ActionRequired.ToString());
        ...
        private async Task<bool> PostNotification(NotificationMessage message, string type) {
            ...
            using (var client = new HttpClient()) {
                ...
                var response =
                    await
                        client.PostAsync(
                            string.Format("lemmy/{0}/{1}/notification/{2}",
                            new Guid(GlobalSettings.LemmyToken), GlobalSettings.LemmyVersion, type),
                            stringContent);
        ...
        public void AnsweringCase_Test() {
            ...
            var cases = Organization.GetCableOwnerCases_Filter(null, 5, null, null, 
                                    new[] {Constants.CABLE_INQUIRY_PURPOSE, Constants.PROJECT_INQUIRY_PURPOSE},
                                    "toconfirm", null, _context).Result;
            ...
            Assert.That(eventModel.Measure.Count, Is.EqualTo(replyEvent.Action.Count), "The actions are not the same as the once sent");
        }

        private async Task<Event> ReplyToCase(string caseNumber, string areaType, 
                                              string recipientId, Reply reply) {
            Event result;
            // Create a client and connect to LK instance
            using (var client = new HttpClient()) {
                // Configure request
