...
namespace TestV4 {
    ...
    public class PostInquiries {
        ...
        private const string _featureCollection =
                "{\"features\":[{\"type\":\"Feature\",\"geometry\":{\"type\":\"Polygon\",\"coordinates\":[[[6401470.1824371638,321765.78432757215],[6401465.8308711154,321790.4632852918],[6401441.199078504,321786.12003563583],[6401445.5506445458,321761.4410779092],[6401470.1824371638,321765.78432757215]]]},\"properties\":{\"CreatedWith\":\"WEBSERVICE\"}}],\"type\":\"FeatureCollection\"}"
            ;
        ...
        private async Task<List<int>> PostAttachments(
            ...
            using (var client = new HttpClient()) {
                ...
                var jsonString = JsonUtils<Dictionary<string, byte[]>>.Serialize(attachments);
                var stringContent = new StringContent(jsonString);
                stringContent.Headers.ContentType = new MediaTypeHeaderValue("application/json");

                // HTTP POST
                var response = await client.PostAsync("inquiry/attachments", stringContent);

                result = JsonUtils<List<int>>.Deserialize(response.Content.ReadAsStringAsync().Result,
        ...
        private async Task<List<InquirerCaseRecipient>> GetInvolvedRecipients
            ...
            using (var client = new HttpClient()) {
                ...
                // HTTP POST
                var url = string.Format("inquiry/{0}/involvedrecipients", typeof(TInput).Name.ToLower());
                var response = await client.PostAsync(url, stringContent);

                result = JsonUtils<List<InquirerCaseRecipient>>.Deserialize(response.Content.ReadAsStringAsync().Result,
                    response.IsSuccessStatusCode);
            }
            return result;
        }
        ...
        private async Task<List<string>> PostInquiry<TOutput, TInput>(
            ...
            using (var client = new HttpClient()) {
                ...
                var url = string.Format("inquiry/{0}", typeof(TOutput).Name.ToLower());
                // HTTP POST
                var response = await client.PostAsync(url, stringContent);

                result = JsonUtils<List<string>>.Deserialize(response.Content.ReadAsStringAsync().Result,
                    response.IsSuccessStatusCode);
            }
            return result;
        }
        ...
        private List<int> AddAttachmentFile() {
            ...
            var dir = new DirectoryInfo(path.Substring(0, path.IndexOf("bin", StringComparison.Ordinal)) +
                                        "TestingFiles\\");
            var post = dir.GetFiles("*.*").ToDictionary(x => x.Name, x => File.ReadAllBytes(x.FullName));

            var attachments = PostAttachments(post).Result;

            Assert.That(attachments.Count, Is.GreaterThanOrEqualTo(1), "There should be one attachment.");
            return attachments;
        }
        ...
        private static void ChecksForCableOwnerCase<TInput>(TInput inquiry) where TInput : Inquiry {
            Assert.That(!string.IsNullOrEmpty(inquiry.Name), Is.True, "Inquiry has no name.");
            Assert.That(!string.IsNullOrEmpty(inquiry.Comment), Is.True, "Inquiry has no comment.");
            Assert.That(!string.IsNullOrEmpty(inquiry.Purpose), Is.True, "Inquiry has no purpose.");
            Assert.That(!string.IsNullOrEmpty(inquiry.CreatedUsing), Is.True, "Inquiry has no createdusing.");
            Assert.That(inquiry.Geometry != null && inquiry.Geometry.Any(), Is.True, "Inquiry has no geometries.");
        }
                ...
                PostInquiry<CableInquiry, CreateCableInquiry>(
                    new CreateCableInquiry {
                        ...
                        Inquiry = new CableInquiry {
                            ...
                            Name = "Testing adding cable indication case through API",
                            Comment = "Testing adding cable indication case through API",
                            // optional
        ...
        public void CreateCollaborationCase() {
            ...
            Assert.That(collaborationInquiryRecipients.Count, Is.GreaterThanOrEqualTo(2),
                ...
                PostInquiry<EmergencyInquiry, CreateEmergencyInquiry>(
                    new CreateEmergencyInquiry {
                        ...
                        Inquiry = new EmergencyInquiry {
                            ...
                            Comment = "Testing adding emergency case through API",
                            Geometry = _featureCollection,
        ...
        public void CreateProjectCase() {
            ...
            if (totalControlOrgs > 1) {
                // so at least 2 => remove half
                var caseRecipients = projectInquiryRecipients.Where(cr => cr.AreaType == "control");
                var index = 0;
                foreach (var cr in caseRecipients) {
                    if (index % 2 == 0) removeInterestedOrgs.Add(cr.Organization.Id);
                    index++;
                }
            }
            var attachments = AddAttachmentFile();
                ...
                PostInquiry<ProjectInquiry, CreateProjectInquiry>(
                    new CreateProjectInquiry {
                        ...
                        Inquiry = new ProjectInquiry {
                            ...
                            Comment = "Testing adding project case through API",
                            Geometry = _featureCollection,
                            ProjectStartDate = DateTime.Today.AddDays(1).ToString("O"),
                            ProjectEndDate = DateTime.Today.AddDays(31).ToString("O"),
                            NotifyOnReply = true,
        ...
        public void PostAttachments() {
            ...
            Assert.That(cableInquiryRecipients.Count, Is.GreaterThanOrEqualTo(2),
                "There should be two case recipients.");
        ...
        public void TestGetWorkCategories() {
            ...
            foreach (var caseType in caseTypesWithWorkCategories) {
                ...
                Assert.That(allCategories.Any(wk => wk.UniqueName.Equals("1_1_VA")));
                Assert.That(allCategories.Any(wk => wk.UniqueName.Equals("2_3_BOSTÄDER")));
            }
