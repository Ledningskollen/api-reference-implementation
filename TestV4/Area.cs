...
namespace TestV4 {
    ...
    public class Areas {
        ...
        private string _featureCollection = @"{""features"":[{""type"":""Feature"",""geometry"":{""type"":""Polygon"",""coordinates"":[[[6301470.1824371638,321765.78432757215],[6301465.8308711154,321790.4632852918],[6301441.199078504,321786.12003563583],[6301445.5506445458,321761.4410779092],[6301470.1824371638,321765.78432757215]]]}}],""type"":""FeatureCollection""}";
        private string _crsFeatureCollection =
        ...
        private string _crsFeatureCollection2 =
        ...
        public void Init() {
            ...
            //RecipientId = Organization.OrganizationRecipients_JSON().Result.First().Id;
        ...
        /// </summary>
        private static async Task<List<Area>> GetArea_JSON(
            string aoiId = null) {
            List<Area> result;
            using (var client = new HttpClient()) {
       ...
       private static async Task<List<Area>> GetAreaType_JSON(
            string areaType) {
            List<Area> result;
            using (var client = new HttpClient()) {
                //Set timeout to 60 seconds
                ...
                var url =
                    string.Format(
                        "organization/{0}/area/{1}",
                        _context.OrganizationId, areaType);
                // HTTP GET
                var response = await client
                    .GetAsync(url);
                result = response.IsSuccessStatusCode
        ...
        private static async Task<Area> AddArea(
            Area area) {
            ...
            using (var client = new HttpClient()) {
                ...
                string aoiJson = JsonUtils<Area>.Serialize(area);
                // HTTP GET
                var response = await client
                    .PostAsync(
                        string.Format("organization/{0}/area",
                            _context.OrganizationId),
                        new StringContent(aoiJson,
                            Encoding.UTF8,
                            "application/json"));
                if (!response.IsSuccessStatusCode) {
                    return null;
                }
                result = JsonUtils<Area>.Deserialize(
                    response.Content.ReadAsStringAsync().Result);
            }
        ...
        private static async Task<Area> PatchArea(
            Area area) {
            Area result;
        ...
        private static async Task<bool> DeleteArea_JSON(string areaId) {
            ...
            using (var client = new HttpClient()) {
                ...
                var response = await client.DeleteAsync(
                    string.Format("organization/{0}/area/{1}",
                        _context.OrganizationId,
                        areaId));
                result = response.IsSuccessStatusCode;
            }
        ...
        public void GetAreasAsJson() {
            ...
            Assert.GreaterOrEqual(areaList.Result.Count,
                1,
                "No areas");
        ...
        public void GetControlAreasAsJson() {
            ...
            Assert.GreaterOrEqual(controlAreaList.Result.Count,
                                  1,
                                  "No control areas");
        ...
        public void GetAffectedAreasAsJson() {
            ...
            Assert.GreaterOrEqual(affectedAreaList.Result.Count,
                                  1,
                                  "No control areas");
        ...
        public void DeleteAndAddAreasAsJson() {
            const string affectedType = "affected"; // This are the areas of interest. They will transformed into a 1km2 area for security
            const string controlType = "control"; // This are the bevakninsområden. They describe the interested areas, but not the where the cables are.
            ...
            var addedControlArea = AddArea(new Area() {
                ...
                Recipients = CreateRecipients(recipientList, controlType, r), //Should be other recipients no emergency,
        ...
        private static void CheckDeleteAddPatchArea(Area originalArea, Area changedArea) {
            Assert.That(originalArea.Geometry.Count(),
                 Is.GreaterThanOrEqualTo(24),
                "Should be 24 polygons in the first grid");

            var addedList = GetArea_JSON().Result;
            Assert.That(addedList.Any(x => x.Id.Equals(originalArea.Id)),
                Is.True,
                "The new geometry hasn't been saved.");

            Assert.That(changedArea.Geometry.Count(),
            ...
            Assert.That(remainderList.Any(x => x.Id.Equals(originalArea.Id)),
                Is.False,
                "The remaining list includes the deleted AOI");
            
        }

        private static Dictionary<string,string> CreateRecipients(List<Recipient> recipientList, string areaType, Random r) {
            ...
            recipients1.Add(caseTypeToAdd, 
                            recipientList.GetRange(r.Next(0, recipientList.Count()), 1).First().Id);
            return recipients1;
        }
        ...
        private void ChecksForAreas(
            IEnumerable<Area> areaList, string areaType = null) {
        ...
        private void ChecksForAreas(Area area) {
            ...
            if (area.AreaType == "affected") {
                Assert.That(area.Recipients.Count,
                            Is.EqualTo(4),
                            "There should be 1 recipient assigned for every case type that affected areas support.");
            } else {// AreaType == "control" Maybe check that collaboration is set?
                Assert.That(area.Recipients.Count,
